...
| 指標 | 件数 |
|---|---|
| PDFの自動発見に成功 | 230/254 (91%) |
| 収益構造・業種別貸出金・有価証券ポートフォリオの3項目すべて取得 | 91/254 (36%) |
| 3項目のうち1つ以上取得 | 189/254 (74%) |
| 収益構造(損益の状況)を取得 | 174/230 |
| 業種別貸出金を取得 | 151/230 |
| 有価証券ポートフォリオを取得 | 116/230 |
| PDFが発見できなかった | 20/254 |
| ダウンロード・解析エラー | 4/254 |

上記は3ラウンドのバグ修正・ヒューリスティック改善を経た数値で、
47金庫サンプル最初の実行（3項目達成38%）から着実に改善している。
バッチを複数回実行すると、同一サイトへの連続アクセスによる一時的な
ブロック(403等)で毎回違う金庫が数件失敗する再現性の低いノイズが
乗るため、この数値は2回分の実行結果から金庫ごとに最良のものを
